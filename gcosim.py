...
def sample_breakpoints(mapDF_c,sex,options):
	...
	gco_breakpoints_physical=[math.floor(x) for y in [[x - np.rint(np.random.normal(loc=options['length']/2, scale=options['length']/50)),x + np.rint(np.random.normal(loc=options['length']/2, scale=options['length']/50))] for x in gco_midpoints_physical ] for x in y ]
	...
	recomb_breakpoints_physical = [math.floor(x) for x in np.interp(recomb_breakpoints_genetic,genetic_positions,physical_positions)] #(note the use of math.floor to round down physical positions from floats to ints )

	all_breakpoints_physical=recomb_breakpoints_physical+gco_breakpoints_physical
	all_breakpoints_physical.sort()

	#Add complex gcos
	complexgco_breakpoints=[]	
	for bp in all_breakpoints_physical:
		dice_roll=random.choice(list(range(options['beta'])))
		if dice_roll == 1:
			complexgco_loc=bp + np.rint(np.random.normal(loc=0, scale=options['dist']))
			complexgco_breakpoints.append(complexgco_loc-np.rint(np.random.normal(loc=options['length']/2, scale=options['length']/50))) #TODO:use truncated normal (or maybe a different distribution)
			complexgco_breakpoints.append(complexgco_loc+np.rint(np.random.normal(loc=options['length']/2, scale=options['length']/50)))
		else:
			continue
	
	#print('complexgco bps : {}'.format(complexgco_breakpoints))
	breakpoints_final=all_breakpoints_physical+complexgco_breakpoints
	breakpoints_final.sort()
...
def transmit_segments(breakpoints,inherited_segments):
	...
	if len(breakpoints)==0:
		...
		for bpno,r in enumerate(breakpoints):
...
class _Node():
	...
	def __init__(self,idno,sex,mapDF):
		...
		self.sex=sex
		self.mapDF=mapDF
		self.gco_params={'dist':5000,'length':1000,'beta':10,'indep_gco_rate':1/3}
		self.bpstr=[[],[]]
	...
	def generate_breakpoints(self):
		...
		for c in range(1,23):
			...
			while not bp_in_map_range: #Resample breakpoints until they fall inside the range of mapfile
				...
				if all([x < mapDF_c.iloc[-1].pos for x in self.breakpoints[c-1]]):
	...
	def generate_bplines(self):
		...
		for h in [0,1]:
			...
			for c in range(1,23):
				...
				str_c = '{}|'.format(c)+str(int(l[0]['start']))+' '+''.join(['{}:{} '.format(d['hap'],int(d['end'])) for d in l])[:-1] #TODO:Convert to integer in transmit_segments rather than here
				bpstr_list.append(str_c + ' ')
				self.bpstr[h]=headers[h] + ' ' + ''.join(bpstr_list)[:-1]
...
class FounderNode(_Node):
	...
	def __init__(self,idno,sex,simno,mapDF):
		super().__init__(idno,sex,mapDF)
		self.simno=simno
		self.inherited_segments=[[[{'start':self.chromosome_endpoints[c-1][0],'end':self.chromosome_endpoints[c-1][1],'hap':str(2*self.idno-1)}],[{'start':self.chromosome_endpoints[c-1][0],'end':self.chromosome_endpoints[c-1][1],'hap':str(2*self.idno)}]] for c in range(1,23)]
		self.generation=0
		...
		while t >= 0:
			...
			for i in range(2**t):
				if i%2:
					...
					tree[7-t].append(ChildNode(i+1,'male',mapDF,[tree[T-t-1][2*i],tree[T-t-1][2*i+1]]))
				else:
					print("creating female : i == {}",format(i))
					tree[7-t].append(ChildNode(i+1,'female',mapDF,[tree[T-t-1][2*i],tree[T-t-1][2*i+1]]))
			t=t-1
		
		tree[T][0].generate_bplines()

		with open(outfile+'_{}.bp'.format(nsim),'w') as f:
			f.write('\n'.join(tree[T][0].bpstr))
